...
-(void)showBigImg{
    ...
    NSString *str =[NSString stringWithFormat:@"%@uploadfiles/%@",URLM,_dataDic[@"ShopMainImg"]];
...
-(void)shareLight{
    ...
    NSString *imageStr=[NSString stringWithFormat:@"%@uploadfiles/%@",URLM,_dataDic[@"ShopImg1"]];
    NSString *Imgstr =[imageStr stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]];
...
-(void)sendComment{
    ...
    view.sendText = ^(NSString *text){
            [_serVice sendCommentShopID:_ID UID:_single.ID ParentID:0 ParentLevel1ID:0  Token:_single.Token CommentInfo:text imageArray:nil SuccessBlock:^(NSString *success){
                i=0;
                [self GetComment];
                [self.tableView reloadData];
            } Failuer:^(NSString *error) {
